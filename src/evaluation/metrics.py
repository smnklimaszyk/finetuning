...
class MedicalDiagnosisEvaluator:
    ...
    def compute_all_metrics(self) -> Dict:
        ...
        metrics = {
            ...
            "prefix_match_1": self.compute_prefix_match_accuracy(
                1
            ),  # Nur Hauptkategorie
        }
...
def evaluate_model(
    model, test_dataset, processor, batch_size: int = 8, config=None
) -> Dict:
    """
    ...
    Returns:
        Dict mit Metriken
    """
    from utils import generate_cache_key, get_cached_predictions, save_predictions_cache
    ...
    use_cache = (
        config
        and hasattr(config.evaluation, "use_prediction_cache")
        and config.evaluation.use_prediction_cache
    )
    force_recompute = (
        config
        and hasattr(config.evaluation, "force_recompute")
        and config.evaluation.force_recompute
    )
    ...
    if use_cache and not force_recompute:
        ...
        max_age = (
            config.evaluation.cache_max_age_days
            if hasattr(config.evaluation, "cache_max_age_days")
            else None
        )
        ...
        if cached_data:
            ...
            logger.info(
                f"   Cached at: {cached_data['metadata'].get('timestamp', 'unknown')}"
            )
    ...
    formatted_inputs = [
        processor.format_dialog_for_inference(conv) for conv in conversations
    ]
